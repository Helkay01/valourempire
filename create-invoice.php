...
<head>
  ...
  <script src="https://cdn.tailwindcss.com"></script>
    ...
    <script src="https://unpkg.com/jspdf-invoice-template@1.4.2/dist/index.js"></script>
...
function downloadPDF() {
  const props = {
    outputType: window.jsPDFInvoiceTemplate.OutputType.Save,
    ...
    contact: {
      label: "Bill To:",
      name: document.getElementById("bill_to").value,
      address: "",
      phone: "",
      email: ""
    },
    invoice: {
      label: "Invoice #",
      num: new Date().toISOString().replace(/[-:.TZ]/g,'').slice(0,14),
      invDate: `Date: ${document.querySelector('input[name="issue_date"]').value}`,
      headerBorder: true,
      tableBodyBorder: true,
      header: [{ title: "#", style: { width: 10 } }, { title: "Item", style: { width: 80 } }, { title: "Qty" }, { title: "Unit Price" }, { title: "Total" }],
      table: [...document.querySelectorAll("#invoice-items tr")].map((row, idx)=>[
        idx+1,
        row.querySelector(".item-name").value,
        row.querySelector(".item-qty").value,
        row.querySelector(".item-price").value,
        (row.querySelector(".item-qty").valueAsNumber * row.querySelector(".item-price").valueAsNumber).toFixed(2)
      ]),
      additionalRows: [
        { col1: "Subtotal:", col2: subtotal.toFixed(2), col3: "₦" },
        { col1: "Discount:", col2: discount.toFixed(2), col3: "₦" },
        { col1: "Total:", col2: total.toFixed(2), col3: "₦", style: { fontSize: 14 } }
      ],
      invDescLabel: "Notes",
      invDesc: "Thank you for your business!"
    },
    footer: { text: "Generated by Your App" },
    pageEnable: true,
    pageLabel: "Page "
  };

  window.jsPDFInvoiceTemplate.default(props);
}
